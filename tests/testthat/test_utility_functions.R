...
test_that("compute_deriv will throw an error if the boundary is too wide for certain density", {
    library(ars)
    expect_error(compute_deriv(99, lb = lb,ub = ub, f = f),
...
test_that("Initialize step choose proper starting points", {
    library(ars)
    expect_gt(initialize_sample(M, -Inf, 8, f = f)$hfamily[1, 3], 0)
    expect_lt(initialize_sample(M, -8, Inf, f = f)$hfamily[2, 3], 0)
    expect_gt(initialize_sample(M, -Inf, Inf, f = f)$hfamily[1, 3], 0)
    expect_lt(initialize_sample(M, -Inf, Inf, f = f)$hfamily[2, 3], 0)
})
...
test_that("initialize_z do not return Inf values", {
    library(ars)
    expect_equal(sum(is.infinite(initialize_sample(M, lb, ub, f = f)$zvalues)),
                 0)
})
